...
win32_directsound_sound_mgr_mono::win32_directsound_sound_mgr_mono(HWND window_handle,
        int sample_bits, int sample_rate, int buffer_length_in_frames, int volume)
		: sound_mgr(sample_bits, sample_rate, buffer_length_in_frames)
{
	...
	lpDSbSecondary->SetVolume(volume);
...
win32_directsound_sound_mgr_mono::~win32_directsound_sound_mgr_mono()
{
	if(lpDSbPrimary)
...
void win32_directsound_sound_mgr_mono::reset()
{
	clear_buffer();
	playing = 0;
}
...
void win32_directsound_sound_mgr_mono::freeze()
{
	if(playing)
	{
		lpDSbSecondary->Stop();
...
void win32_directsound_sound_mgr_mono::thaw()
{
	if(!playing)
	{
		clear_buffer();
...
void win32_directsound_sound_mgr_mono::clear_buffer()
{
	...
	{
		LOG("Error locking DirectSound buffer in win32_directsound_sound_mgr_mono::clear_buffer()" << endl);
		return;
	}
	...
	if(FAILED(lpDSbSecondary->Unlock((void*)audio_ptr_1,
	                                 ...
	                                 audio_length_2)))
	{
		LOG("Error unlocking DirectSound buffer in win32_directsound_sound_mgr_mono::clear_buffer()" << endl);
		return;
	}
}

boolean win32_directsound_sound_mgr_mono::lock(sound_buf_pos which, void** buf, uint32* buf_len)
{
	HRESULT result;
	...
	if(FAILED(result))
	{
		LOG("Error locking DirectSound buffer in win32_directsound_sound_mgr_mono::lock()" << endl);
		return FALSE;
	}
...
void win32_directsound_sound_mgr_mono::unlock()
{
...
bool win32_directsound_sound_mgr_mono::start_sndrec( char *fn, uint32 nSampleBits, uint32 nSampleRate)
{
	rec = new CSndRec( fn, nSampleBits, nSampleRate, 1);
	if(!rec)
		return false;
	if(rec->status==0){
		delete rec;
...
void win32_directsound_sound_mgr_mono::end_sndrec()
{
	if(rec){
		delete rec;
		rec = NULL;
...
bool win32_directsound_sound_mgr_mono::IsRecording()
{
...
sound_buf_pos win32_directsound_sound_mgr_mono::get_currently_playing_half()
{
	DWORD play_pos;
	DWORD write_pos;
