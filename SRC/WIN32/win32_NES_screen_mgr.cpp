...
boolean win32_NES_screen_mgr::GoWindowed()
{
	...
	try {
		if(NESTER_settings.nes.graphics.WindowModeUseDdraw){
			if(screen)
				delete screen;
			sm = new win32_overlaywindowed_NES_screen_mgr(wnd_handle);
		}
		else{
...
boolean win32_NES_screen_mgr::GoFullscreen()
{
	...
	try {
		...
		sm = new win32_nnnfullscreen_NES_screen_mgr(wnd_handle,
...
void win32_NES_screen_mgr::SetScreenmgr(){
	NES_screen_mgr* sm;
	if(screen){
		delete screen;
		screen=NULL;
	...
	try {
		if(fullscreen){
			//		  if(NESTER_settings.nes.graphics.FullscreenModeUseNewMode){
			sm = new win32_nnnfullscreen_NES_screen_mgr(wnd_handle,GetGUIDPtr(&NESTER_settings.nes.graphics.osd.device_GUID),
			        NESTER_settings.nes.graphics.fullscreen_scaling);
			/*		  }
					  else{
						  sm = new win32_fullscreen_NES_screen_mgr(wnd_handle, GetGUIDPtr(&NESTER_settings.nes.graphics.osd.device_GUID),
				                                             NESTER_settings.nes.graphics.fullscreen_scaling);
					  }
			*/	  }
		else{
...
void win32_NES_screen_mgr::nnnManuEnable(unsigned char flag){
	screen->nnnManuEnable(flag);
}


void win32_NES_screen_mgr::EndEmu(){
	screen->EndEmu();
}


void win32_NES_screen_mgr::StartEmu(){
	screen->StartEmu();
}
...
/*
void win32_NES_screen_mgr::set_screenmode(unsigned char sm)
{
	if(ScreenMode != sm){
		ScreenMode = sm;
