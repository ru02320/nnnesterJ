...
#define NESROM_ID "NNN_ROM"
#define NES_DESCRIPTION "NES_ROM"
#define NESROM_EXTENSION ".nes"

#define FDS_ID "NNN_FDS"
#define FDS_DESCRIPTION "FDS"
#define FDSROM_EXTENSION ".fds"



boolean AssociateExtension(char *ext, char *id, char *desc, int nIcon)
{
	...
	try {
		...
		// open the .NES key
		if(RegCreateKey(HKEY_CLASSES_ROOT, ext, &dotNES_key) != ERROR_SUCCESS)
			throw -1;

		// set the app ID string
		if(RegSetValue(dotNES_key, NULL, REG_SZ, id, strlen(id)) != ERROR_SUCCESS)
			throw -1;
		...
		if(RegCreateKey(HKEY_CLASSES_ROOT, id, &nesterID_key) != ERROR_SUCCESS)
			throw -1;

		// set the document description string
		if(RegSetValue(nesterID_key, NULL, REG_SZ, desc,
		               strlen(desc)) != ERROR_SUCCESS)
			throw -1;
		...
		// set the default icon string
		//strcpy(defIcon_str, full_exe_name);
		sprintf( defIcon_str, "%s,%d",full_exe_name, nIcon);
...
boolean AssociateNESExtension(){
	return AssociateExtension(NESROM_EXTENSION, NESROM_ID, NES_DESCRIPTION, 1);
}


boolean AssociateFDSExtension(){
	return AssociateExtension(FDSROM_EXTENSION, FDS_ID, FDS_DESCRIPTION, 2);
}


void UndoAssociateExtension(char *ext, char *id){
	// delete the .NES key
	RegDeleteKey(HKEY_CLASSES_ROOT, ext);

	// delete the nesterID key
	RegDeleteKey(HKEY_CLASSES_ROOT, id);
...
{
	UndoAssociateExtension(NESROM_EXTENSION, NESROM_ID);
}


void UndoAssociateFDSExtension()
{
	UndoAssociateExtension(FDSROM_EXTENSION, FDS_ID);
}
...
int CheckAssociateExtension(char *ext, char *id){
	HKEY hKey;
	...
	if(RegOpenKeyEx(HKEY_CLASSES_ROOT, ext, 0, 0, &hKey) != ERROR_SUCCESS)
		return 0;
	Size = sizeof(buff);
	if(RegQueryValueEx(hKey, NULL, 0, &Type, buff, &Size) != ERROR_SUCCESS){
		RegCloseKey(hKey);
		return 0;
	}
//	REG_SZ
	if(!strcmp(id, (const char *)buff)){
		RegCloseKey(hKey);
		return 1;
	}

//	if(RegSetValue(dotEXT_key, NULL, REG_SZ, APP_ID, strlen(APP_ID)) != ERROR_SUCCESS)
//		goto errlbl;
...
int CheckAssociateNESExtension(){
	return CheckAssociateExtension(NESROM_EXTENSION, NESROM_ID);
}


int CheckAssociateFDSExtension(){
	return CheckAssociateExtension(FDSROM_EXTENSION, FDS_ID);
}
