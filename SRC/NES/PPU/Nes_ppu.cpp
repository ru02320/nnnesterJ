...
#define VRAM_R(addr) \
  (PPU_VRAM_banks[((addr | dwAdd_OnyoniPuu) >> 10 )][(addr | dwAdd_OnyoniPuu) & 0x3FF] | ucDat_OnyoniPuu )

#define VRAM_W(addr, data) \
  PPU_VRAM_banks[((addr | dwAdd_OnyoniPuu) >> 10 )][(addr | dwAdd_OnyoniPuu) & 0x3FF] = data | ucDat_OnyoniPuu
...
#define VRAM_W(addr, data) \
  PPU_VRAM_banks[((addr) >> 10 )][(addr) & 0x3FF] = data
...
void NES_PPU::render_bg(uint8* buf, float CYCLES_PER_DRAW)
{
  ...
  for(i = 33; i; i--)
  {
    ...
    try
    {
      pattern_addr = bg_pattern_table_addr + ((int32)VRAM_R(name_addr) << 4) + ((loopy_v & 0x7000) >> 12);
  ...
  if(!NESTER_settings.nes.graphics.DisableBackGClipping && bg_clip_left8())
  {
    // clip left 8 pixels
...
void NES_PPU::render_spr(uint8* buf)
{
  ...
  for(s = 0; s < 64; s++)
  {
    ...
    // clip left
    if((spr_x < 8) && !NESTER_settings.nes.graphics.DisableSpriteClipping && (spr_clip_left8()))
    {
      if(0 == spr_x) continue;
      start_x += (8 - spr_x);
    }
    ...
    for(x = start_x; x != end_x; x += inc_x)
    {
      ...
      if(!((*solid) & SPR_WRITTEN_FLAG))
      {
        ...
        if(VRAM_R(tile_addr) & tile_mask) col |= 0x01;
        tile_addr += 8;
        if(VRAM_R(tile_addr) & tile_mask) col |= 0x02;
