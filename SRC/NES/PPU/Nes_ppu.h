...
public:
  ...
  static int getTopMargin(unsigned char c)      { return (NESTER_settings.nes.graphics.show_all_scanlines||c==2) ? 0 : 8; }
//  int getTopMargin(){ return (NESTER_settings.nes.graphics.show_all_scanlines) ? 0 : 8; }
  static int getViewableHeight(unsigned char c){	return NES_SCREEN_HEIGHT-(2*getTopMargin(c));}
