...
static void adopt_ExMPRD(const char* fn, NES* nes)
{
	...
	fseek(fp, 4, SEEK_SET);
	uint32 block_num = 0;
	block_num |= fgetc(fp) << 24;
	block_num |= fgetc(fp) << 16;
	block_num |= fgetc(fp) << 8;
	block_num |= fgetc(fp) << 0;
	for(i = 0; i < block_num; i++)
	{
		fseek(fp, 8, SEEK_CUR);
		uint32 block_length = 0;
		block_length |= fgetc(fp) << 24;
		block_length |= fgetc(fp) << 16;
		block_length |= fgetc(fp) << 8;
		block_length |= fgetc(fp) << 0;
		fseek(fp, block_length, SEEK_CUR);
	}

	if(nes->ROM->get_mapper_num() == 1 && nes->crc32() == 0xb8747abf)
	{
...
/*
		...
		{
			nes->SaveRAM[i] = fgetc(fp);
		}*/
		fread(&nes->SaveRAM[0x2000], 0x2000, 1, fp);
	}
	else if(nes->ROM->get_mapper_num() == 5)
	{
		NES_mapper5* mapper = (NES_mapper5*)nes->mapper;
...
/*		for(i = 0; i < 0x10000; i++)
		{
			mapper->wram[i] = nes->SaveRAM[i] = fgetc(fp);
		}*/
		fread(mapper->wram, 0x10000, 1, fp);
	}
	else if(nes->ROM->get_mapper_num() == 20)
	{
		NES_mapper20* mapper = (NES_mapper20*)nes->mapper;
...
/*
		for(i = 0; i < 0x8000; i++)
		{
			mapper->wram[i] = fgetc(fp);
		}*/
		fread(mapper->wram, 0x8000, 1, fp);

		// read Disk #1 - #4
		fseek(fp, 12, SEEK_CUR);
/*		for(i = 0; i < 65500; i++)
		{
			mapper->disk[0x00000+i] = fgetc(fp);
		}
		fseek(fp, 12, SEEK_CUR);
		for(i = 0; i < 65500; i++)
		{
			mapper->disk[0x10000+i] = fgetc(fp);
		}
		fseek(fp, 12, SEEK_CUR);
		for(i = 0; i < 65500; i++)
		{
			mapper->disk[0x20000+i] = fgetc(fp);
		}
		fseek(fp, 12, SEEK_CUR);
		for(i = 0; i < 65500; i++)
		{
			mapper->disk[0x30000+i] = fgetc(fp);
		}*/
		fread(mapper->disk, 65500*4, 1, fp);
	}
...
static int extract_VRAM(SnssVramBlock* block, NES* nes)
{
	...
	DWORD *dp = (DWORD *)patterntables;
	for(uint32 i = 0; i < 0x8000/4; i++)
	{
		if(dp[i] != 0x00) break;
	}
	if(i == 0x8000/4) return -1;

	// 8K of VRAM data
	block->vramSize = nes->ppu->vram_size;
...
static int extract_SRAM(SnssSramBlock* block, NES* nes)
{
	...
	DWORD *dp = (DWORD *)nes->SaveRAM;
	for(uint32 i = 0; i < sizeof(nes->SaveRAM)/4; i++)
	{
		if(dp[i] != 0x00) break;
	}
	if(i == sizeof(nes->SaveRAM)/4) return -1;
...
static void extract_ExMPRD(const char* fn, NES* nes)
{
	...
	if(nes->ROM->get_mapper_num() == 1 && nes->crc32() == 0xb8747abf)
	{
		...
		// save WRAM
		fputc('W',  fp); fputc('R',  fp); fputc('A',  fp); fputc('M',  fp);
		fputc(0x00, fp); fputc(0x00, fp); fputc(0x00, fp); fputc(0x01, fp);
		fputc(0x00, fp); fputc(0x01, fp); fputc(0x00, fp); fputc(0x00, fp);
#if 0
		for(i = 0x2000; i < 0x4000; i++)
		{
			fputc(nes->SaveRAM[i], fp);
		}
#else
		fwrite(&nes->SaveRAM[0x2000], 1, 0x2000, fp);
#endif
	}
	else if(nes->ROM->get_mapper_num() == 5)
	{
		...
		for(i = 0; i < 0x10000; i++)
		{
			fputc(mapper->wram[i], fp);
		}
#else
		fwrite(mapper->wram, 1, 0x10000, fp);
#endif
	}
	else if(nes->ROM->get_mapper_num() == 20)
	{
		...
		fputc('W',  fp); fputc('R',  fp); fputc('A',  fp); fputc('M',  fp);
		fputc(0x00, fp); fputc(0x00, fp); fputc(0x00, fp); fputc(0x01, fp);
		fputc(0x00, fp); fputc(0x00, fp); fputc(0x80, fp); fputc(0x00, fp);
#if 0
		for(i = 0; i < 0x8000; i++)
		{
			fputc(mapper->wram[i], fp);
...
#else
		fwrite(mapper->wram, 1, 0x8000, fp);
#endif
		// write Disk #1 - #4
		fputc('F',  fp); fputc('D',  fp); fputc('S',  fp); fputc('1',  fp);
		fputc(0x00, fp); fputc(0x00, fp); fputc(0x00, fp); fputc(0x01, fp);
		fputc(0x00, fp); fputc(0x00, fp); fputc(0xFF, fp); fputc(0xDC, fp);
		fwrite(mapper->disk, 65500*4, 1, fp);
#if 0
		for(i = 0; i < 65500; i++)
		{
			fputc(mapper->disk[0x00000+i], fp);
		}
#else
//		fwrite(mapper->disk, 1, 65500, fp);
#endif
		fputc('F',  fp); fputc('D',  fp); fputc('S',  fp); fputc('2',  fp);
		fputc(0x00, fp); fputc(0x00, fp); fputc(0x00, fp); fputc(0x01, fp);
		fputc(0x00, fp); fputc(0x00, fp); fputc(0xFF, fp); fputc(0xDC, fp);
#if 0
		for(i = 0; i < 65500; i++)
		{
			fputc(mapper->disk[0x10000+i], fp);
		}
#else
//		fwrite(&mapper->disk[0x10000], 1, 65500, fp);
#endif
		fputc('F',  fp); fputc('D',  fp); fputc('S',  fp); fputc('3',  fp);
		fputc(0x00, fp); fputc(0x00, fp); fputc(0x00, fp); fputc(0x01, fp);
		fputc(0x00, fp); fputc(0x00, fp); fputc(0xFF, fp); fputc(0xDC, fp);
#if 0
		for(i = 0; i < 65500; i++)
		{
			fputc(mapper->disk[0x20000+i], fp);
		}
#else
//		fwrite(&mapper->disk[0x20000], 1, 65500, fp);
#endif
		fputc('F',  fp); fputc('D',  fp); fputc('S',  fp); fputc('4',  fp);
		fputc(0x00, fp); fputc(0x00, fp); fputc(0x00, fp); fputc(0x01, fp);
		fputc(0x00, fp); fputc(0x00, fp); fputc(0xFF, fp); fputc(0xDC, fp);
#if 0
		for(i = 0; i < 65500; i++)
		{
			fputc(mapper->disk[0x30000+i], fp);
		}
#else
//		fwrite(&mapper->disk[0x30000], 1, 65500, fp);
#endif
	}
	fclose(fp);
}
...
inline void SetStateChunkHeader(DWORD *Memp, int Code, int Size){
	Memp[0] = Code;
	Memp[1] = Size;
	Memp[2] = 0;
	Memp[3] = 0;
...
static int extractMem_ExMPRD(unsigned char **extm, uint32 nsize, NES* nes)
{
	...
	if(nes->ROM->get_mapper_num() == 1 && nes->crc32() == 0xb8747abf)
	{
		...
		if(tmem==NULL)
			return 0;
		extp=tmem+nsize;
		SetStateChunkHeader((DWORD *)extp, 0x44504D45, 0x2000);
		memcpy(extm+0x10, &nes->SaveRAM[0x2000], 0x2000);
		size = 0x2000+0x10;
	}
	else if(nes->ROM->get_mapper_num() == 5)
	{
		...
		if(tmem==NULL)
			return 0;
		extp=tmem+nsize;
		SetStateChunkHeader((DWORD *)extp, 0x44504D45, 0x10000);
		memcpy(extp+0x10, mapper->wram, 0x10000);
		size = 0x10000+0x10;
	}
	else if(nes->ROM->get_mapper_num() == 20)
	{
		...
		if(tmem==NULL)
			return 0;
		extp=tmem+nsize;
		SetStateChunkHeader((DWORD *)extp, 0x44504D45, 0x8000+(65500*4));
		memcpy(extp+0x10, mapper->wram, 0x8000);
		extp+=0x10;

		// write Disk #1 - #4
		memcpy(&extp[0x8000], mapper->disk, 65500);
		memcpy(&extp[0x8000+65500], &mapper->disk[0x10000], 65500);
		memcpy(&extp[0x8000+65500*2], &mapper->disk[0x20000], 65500);
		memcpy(&extp[0x8000+65500*3], &mapper->disk[0x30000], 65500);
...
static int adoptMem_ExMPRD(unsigned char *extm, uint32 nsize, NES* nes)
{
	...
	if(nes->ROM->get_mapper_num() == 1 && nes->crc32() == 0xb8747abf)
	{
		// Best Play - Pro Yakyuu Special (J)
		// copy WRAM
		memcpy(&nes->SaveRAM[0x2000],extm, 0x2000);
		size = 0x2000;
	}
	else if(nes->ROM->get_mapper_num() == 5)
	{
		...
		// copy WRAM
		memcpy(mapper->wram, extm, 0x10000);
		memcpy(nes->SaveRAM, extm, 0x10000);
		size = 0x10000;
	}
	else if(nes->ROM->get_mapper_num() == 20)
	{
		...
		// copy WRAM
		memcpy(mapper->wram, extm, 0x8000);
		// read Disk #1 - #4
		memcpy(mapper->disk, &extm[0x8000], 65500);
		memcpy(&mapper->disk[0x10000], &extm[0x8000+65500], 65500);
		memcpy(&mapper->disk[0x20000], &extm[0x8000+65500*2], 65500);
		memcpy(&mapper->disk[0x30000], &extm[0x8000+65500*3], 65500);
		size = 0x8000+65500*4;
	}
	return size;
}
...
int MemSaveSNSS(unsigned char **ssp, NES* nes){
	...
	*((DWORD *)(void *)&snss[0]) = 0x54445453;
	*((DWORD *)(void *)&snss[4]) = ssize;
	*ssp = snss;
...
int MemLoadSNSS(unsigned char *ssp , NES* nes){
	...
	if(*(DWORD *)&ssp[0]!=0x54445453) return 0;
	tsize = *((DWORD *)&ssp[4])+0x20;
	snss+=0x20;
	lsize = 0x20;
	while(lsize < tsize){
		switch(*(DWORD *)snss){
			...
			case 0x4E554F53:
				...
				break;
			case 0x44504D45:
				adoptMem_ExMPRD(snss+0x10, *(DWORD *)&snss[4], nes);
				break;
		}
		DWORD tdw = (*(DWORD *)&snss[4])+0x10;
		snss+=tdw;
		lsize+=tdw;
/*
		snss+=((*(DWORD *)&snss[4])+0x10);
		lsize+=((*(DWORD *)&snss[4])+0x10);
*/
	}
	return lsize;
}
