...
void NES_mapper20::Reset()
{
	...
	parent_NES->mapper_extramsize = 0x8000;
...
//	bios[0xfb0] = 0x00;
	...
	{
		memcpy(disk, &ROM_banks[16], ROM_banks[4]*65500);
	}
	else
	{
	...
	else if(parent_NES->fds_id() == 0x01534d42) // SMB2
	{
		irq_cdec = 112;
	}
/*	else if(parent_NES->fds_id() == 0xb2445052){
		irq_cdec = 112;
	}
*/
	irq_enabled = 0;
	...
	case 0x4031:
		{
			...
			{
				val = disk[(current_side-1)*65500+head_position];
				if(write_reg & 0x01)
				{
	...
	case 0x4022:
		{
			...
			irq_enabled = (data & 0x03) && (disk_enabled);
		}
		break;
	...
	case 0x4024:
		{
			if((current_side != 0) && (current_side == last_side))
			{
				if(disk_enabled && !(write_reg & 0x04) && head_position < 65000)
				{
					...
					{
						disk[(current_side-1)*65500+head_position-2] = data;
						access_flag = 1;
					}
				}
			}
