...
void NES_mapper19::Reset()
{
	...
	if(parent_NES->crc32() == 0x761ccfb5 )//|| // Digital Devil Story - Megami Tensei 2
		//	  parent_NES->crc32() == 0x3deac303)  // RollingThunder(J)
	{
		patch = 1;
		irq_sn = 112;
	}
	else if(parent_NES->crc32() == 0x96533999 || // Dokuganryuu Masamune
	...
	if(parent_NES->crc32() == 0xb62a7b71) // Family Circuit '91(J)
	{
		irq_sn=100;
	}
