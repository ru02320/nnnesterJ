...
NES_mapper::NES_mapper(NES* parent) : parent_NES(parent)
{
	...
	if(parent_NES->ROM->get_unifmapper_num()){
		num_16k_ROM_banks = parent_NES->ROM->get_UNIF_psize_16k();
		num_8k_ROM_banks = num_16k_ROM_banks<<1;
		num_1k_VROM_banks = parent_NES->ROM->get_UNIF_csize_8k()<<3;
	}
	else{
...
void NES_mapper::set_CPU_bank(uint8 pagen, uint32 kn){
	NES_6502::Context context;
	VALIDATE_ROM_BANK(kn);
	parent_NES->cpu->GetContext(&context);
	context.mem_page[pagen] = ROM_banks + (kn << 13); // * 0x2000
	parent_NES->cpu->SetContext(&context);
	set_genie();
}
...
// for UNIF
void NES_mapper::set_CPU_bank_unif(uint32 kn, uint8 pagen){
	NES_6502::Context context;
	VALIDATE_ROM_BANK(kn);
...
void NES_mapper::set_VRAM_bank_unif(uint32 kn, uint8 pagen){
	VALIDATE_VROM_BANK(kn);
	parent_NES->ppu->PPU_VRAM_banks[pagen] = VROM_banks + (kn << 10); // * 0x400
...
void NES_mapper::set_PPU_bank(uint8 pagen, uint32 kn){
	VALIDATE_VROM_BANK(kn);
	if(pagen < 8)
		parent_NES->ppu->PPU_VRAM_banks[pagen] = VROM_banks + (kn << 10); // * 0x400
}
...
{
	if(rom->get_unifmapper_num()){
		switch(rom->get_unifmapper_num()){
			case 1:
				return new NES_UNIFmapper1(parent);
			case 2:
				return new NES_UNIFmapper2(parent);
			case 3:
				return new NES_UNIFmapper3(parent);
			case 4:
				return new NES_UNIFmapper4(parent);
			default:
				return NULL;
		}
	}
	...
	case 62:
		return new NES_mapper62(parent);
	...
	case 74:
		return new NES_mapper74(parent);
	...
	case 114:
		return new NES_mapper114(parent);

	case 115:
		return new NES_mapper115(parent);
	...
	case 140:
		return new NES_mapper140(parent);

	case 151:
		return new NES_mapper151(parent);

	case 155:
		return new NES_mapper155(parent);
	...
	case 200:
		return new NES_mapper200(parent);

	case 201:
		return new NES_mapper201(parent);

	case 202:
		return new NES_mapper202(parent);

	case 203:
		return new NES_mapper203(parent);

	case 204:
		return new NES_mapper204(parent);

	case 205:
		return new NES_mapper205(parent);

	case 211:
		return new NES_mapper211(parent);

	case 212:
		return new NES_mapper212(parent);

	case 215:
		return new NES_mapper215(parent);

	case 217:
		return new NES_mapper217(parent);

	case 222:
		return new NES_mapper222(parent);
	...
	case 241:
		return new NES_mapper241(parent);
	...
	case 244:
		return new NES_mapper244(parent);
	...
	case 249:
		return new NES_mapper249(parent);

	case 250:
		return new NES_mapper250(parent);

	case 251:
		return new NES_mapper251(parent);

	case 252:
		return new NES_mapper252(parent);

	case 254:
		return new NES_mapper254(parent);
