...
static int32 FDSSoundRender(void)
{
	...
	{
		uint32 sweeps=(uint32)fds_sound.op[1].sweep;
		pop->envphase++;
		if (!(pop->envmode & 0x40))
		{
...
static void FDSSoundWrite(uint32 address, uint8 value)
{
	...
	{
		fds_sound.op[1].wave[address - 0x4040] = LinearToLog(((int32)value & 0x3f) - 0x20);
	}
	else if (0x4080 <= address && address <= 0x408F)
	{
		...
		switch (address & 15)
		{
			...
			case 4:
				if(value & 0x80)
				{
					int32 a=fds_sound.op[1].spd;
					int32 b=fds_sound.op[1].sweep;
			...
			case 1:
				if ((value & 0x7f) < 0x60)
					...
					fds_sound.op[0].sweep = ((int32)value & 0x7f) - 0x80;
				break;
			case 5:
				if (!value) break;
				if ((value & 0x7f) < 0x60)
				{
					fds_sound.op[1].sweep = (int32)fds_sound.op[1].spd+
						((fds_sound.op[1].spd * (value & 0x7f))>>5);
				}
				else
				{
					fds_sound.op[1].sweep = (int32)fds_sound.op[1].spd-
						(((fds_sound.op[1].spd) * (((int32)value & 0x7f) - 0x80)) >> 5);
				}
				break;
			case 2:
...
static void FDSSoundReset(void)
{
	int8 i;
	FDS_FMOP *pop;
	memset(&fds_sound, 0, sizeof(FDSSOUND));
...
static void fds_dll_snd_reset(void){
	if(hVirtuaNES_FDS_DLL){
		if(hVirtuaNES_FDS){
			if(FDS_Reset != NULL){
				FDS_Reset(hVirtuaNES_FDS, apu->sample_rate);
			}
		}
	}
}
...
static void fds_dll_snd_paramschanged(void){
	if(hVirtuaNES_FDS_DLL){
		if(hVirtuaNES_FDS){
			if(FDS_Setup != NULL){
				FDS_Setup(hVirtuaNES_FDS, apu->sample_rate);
			}
		}
	}
}
