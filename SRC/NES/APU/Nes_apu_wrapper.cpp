...
void NES_APU::AssertParams()
{
  if(apu)
  {
	...
	apu_setvolumed(0, NESTER_settings.nes.sound.rectangle1_volumed);
	apu_setvolumed(1, NESTER_settings.nes.sound.rectangle2_volumed);
	apu_setvolumed(2, NESTER_settings.nes.sound.triangle_volumed);
	apu_setvolumed(3, NESTER_settings.nes.sound.noise_volumed);
	apu_setvolumed(4, NESTER_settings.nes.sound.dpcm_volumed);
	apu_setvolumed(5, NESTER_settings.nes.sound.ext_volumed);
    ...
    switch(NESTER_settings.nes.sound.filter_type)
    {
      ...
      case NES_sound_settings::FILTER_LOWPASS_WEIGHTED:
        ...
        break;
      case NES_sound_settings::FILTER_OLOWPASS:
...
#endif
	apu_setlowpass_filter_f(NESTER_settings.nes.sound.lowpass_filter_f);
  }
}
...
*/
  if(data & 1){		// VRC6
	  if(NESTER_settings.nes.sound.vrc6_type == 0)
		  apu->ext[0] = &vrc6_ext;  // (nes_vrc6.cpp)	<uonester>
	  else
		  apu->ext[0] = &vrcvi_ext;	// (vrcvisnd.c)	<nester public bata4>
	  if(apu->ext[0]->init)
		  apu->ext[0]->init();
  }
  if(data & 2){		// VRC7
	  if(NESTER_settings.nes.sound.vrc7_type == 0)
		  apu->ext[1] = &vrc7_ext;	// (nes_vrc7.cpp)	<uonester>
	  else
		  apu->ext[1] = &emu2413_ext;
	  if(apu->ext[1]->init)
		  apu->ext[1]->init();
  }
  if(data & 4){		// FDC
	  if(fds_dll_ext.init()){
		  apu->ext[4] = &fds_dll_ext;
	  }
	  else{
		  apu->ext[4] = &fds_nesp_ext;
		  if(apu->ext[4]->init)
			  apu->ext[4]->init();
	  }
  }
