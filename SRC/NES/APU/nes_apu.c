...
static FILE *fpSoundLog = NULL;
static int LogFirst = 0;
static int32 LogLastTime = 0;



boolean apu_start_apu_is_log(){
	if(fpSoundLog != NULL)
		return TRUE;
	return FALSE;
}

void apu_stop_apu_log(){
	if(fpSoundLog){
		fclose(fpSoundLog);
		fpSoundLog = NULL;
	}
...
int apu_start_apu_log(char *fn, char **pptext){
	uint8 buffer[8];
	apu_stop_apu_log();
	fpSoundLog = fopen(fn, "wb");
	if(fpSoundLog == NULL){
		return 0;
	}

	memset(buffer, 0, 8);
	*((uint32 *)(&buffer[0])) = 0x004c534e;	// "NSL"
	buffer[4] = apu->ex_chip;
	fwrite(buffer, 8, 1, fpSoundLog);

	if(pptext){
		uint32 i=2, j;

		buffer[0] = 0;
		fwrite(buffer, 1, 1, fpSoundLog);		// crc32 id(0)
		fwrite(&pptext[0], 4, 1, fpSoundLog);	// crc32 4byte

		buffer[0] = 1;
		buffer[1] = 0x16;
		fwrite(buffer, 2, 1, fpSoundLog);		// regs id(1) 22byte
		fwrite(pptext[1], 0x16, 1, fpSoundLog);

		for(; pptext[i]; i++){
			buffer[0] = i+0x10-2;					// text id 0x10 -> 
			j = strlen(pptext[i]);
			buffer[1] = (uint8)j;				// text size 1byte
			fwrite(buffer, 2, 1, fpSoundLog);	// id write
			fwrite(pptext[i], j, 1, fpSoundLog);// text
		}
	}
	buffer[0] = 0xff;
	fwrite(buffer, 1, 1, fpSoundLog);
	return 1;
}


static void WriteAPULog(apudata_t *d){
	uint8 buffer[8];

	if(!LogFirst){
		LogLastTime = d->timestamp;
		*((uint32 *)(&buffer[0])) = 0;
		LogFirst = 1;
	}
	else {
		*((uint32 *)(&buffer[0])) =  d->timestamp - LogLastTime;
//		LogLastTime = d->timestamp;
	}

	*((uint16 *)(&buffer[4])) = (uint16)d->address;
	*((uint8  *)(&buffer[6])) = d->value;

	fwrite(buffer, 7, 1, fpSoundLog);
}
   ...
   default:
	   //DCR
	   {
		   ...
		   for(i=0; i<NESAPU_EXCHIP_MAX; i++){
			   if(apu->ext[i])
				   if(apu->ext[i]->mem_write){
					   apu_memwrite* mw = apu->ext[i]->mem_write;
...
//    return 0x00;
	...
	//DCR
	{
		...
		for(i=0; i<NESAPU_EXCHIP_MAX; i++){
			if(apu->ext[i])
				if(apu->ext[i]->mem_read){
					apu_memread* mr = apu->ext[i]->mem_read;
   ...
   //DCR
	{
		...
		for(i=0; i<NESAPU_EXCHIP_MAX; i++){
			if(apu->ext[i])
				if(apu->ext[i]->mem_writesync){
					apu_memwrite* mw = apu->ext[i]->mem_writesync;
					while(mw->max_range != -1){
						if((mw->min_range <= address) && ((mw->max_range >= address))){
							mw->write_func(address, value);
							break;
...
void apu_process(void *buffer, int num_samples)
{
   ...
   else
   {
      ...
      while (num_samples--)
      {
         ...
         accum = 0;
         if (apu->mix_enable[0]){
			 if(apu->mix_volumed[0])
				accum += (int)(apu_rectangle(&apu->apus.rectangle[0])/apu->mix_volumed[0]);
			 else
				accum += apu_rectangle(&apu->apus.rectangle[0]);
		 }
         if (apu->mix_enable[1]){
			 if(apu->mix_volumed[1])
				 accum += (int)(apu_rectangle(&apu->apus.rectangle[1])/apu->mix_volumed[1]);
			 else
				 accum += apu_rectangle(&apu->apus.rectangle[1]);
		 }
         if (apu->mix_enable[2]){
			 if(apu->mix_volumed[2])
				 accum += (int)(apu_triangle(&apu->apus.triangle)/apu->mix_volumed[2]);
			 else
				 accum += apu_triangle(&apu->apus.triangle);
		 }
         if (apu->mix_enable[3]){
			 if(apu->mix_volumed[3])
				 accum += (int)(apu_noise(&apu->apus.noise)/apu->mix_volumed[3]);
			 else
				 accum += apu_noise(&apu->apus.noise);
		 }
		 if (apu->mix_enable[4]){
			 if(apu->mix_volumed[4])
				 accum += (int)(apu_dmc(&apu->apus.dmc)/apu->mix_volumed[4]);
			 else
         ...
         if (apu->mix_enable[5])
         {
			int taccum=0;
			{
				...
				for(i=0; i<NESAPU_EXCHIP_MAX; i++){
					if(apu->ext[i])
						if(apu->ext[i]->process){
							taccum += apu->ext[i]->process();
						}
				}
			}
			if(apu->mix_volumed[5])
				accum+=(int)(taccum/apu->mix_volumed[5]);
			else
				accum+=taccum;
         }
         ...
         if (APU_FILTER_NONE != apu->filter_type)
         {
			...
			else if(apu->lowpass_filter_f){
				static int in1=0, in2=0, out1=0, out2=0;
				double r = sqrt(2);
				double c = 1.0/tan(3.14159265*apu->lowpass_filter_f/apu->sample_rate);
...
void apu_setlowpass_filter_f(int lowpass_filter_f){
	ASSERT(apu);
	apu->lowpass_filter_f = lowpass_filter_f;
}
...
//DCR   apu_reset();
   {
	   ...
	   for(i=0; i<NESAPU_EXCHIP_MAX; i++){
		   if(apu->ext[i])
			   if(apu->ext[i]->paramschanged)
				   apu->ext[i]->paramschanged();
	   }
   }
}
...
void apu_destroy(apu_t **src_apu)
{
   if(*src_apu){
	   ...
	   for(i=0; i<NESAPU_EXCHIP_MAX; i++){
		   if((*src_apu)->ext[i])
			   if((*src_apu)->ext[i]->shutdown)
				   (*src_apu)->ext[i]->shutdown();
	   }
 	   free(*src_apu);
   }
   apu_stop_apu_log();
