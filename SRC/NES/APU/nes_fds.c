...
void FDS_SetClock(double c){
	FDS_SOUND.clock = c / 12;
}


void FDS_SetRate (double r){
...
void counter_init(double clk, double rate, uint32 m){
	COUNTER_SHIFT = 24;
	val = 0;
	step = 0;
...
static void update_envelope(int ch){
	while (FDS_SOUND.ecounter[ch] >= 1.0){
...
static int32 calc(){
	...
	while (FDS_SOUND.pcounter[1] >= 1.0)
	{
	...
	while (FDS_SOUND.pcounter[0] >= 1.0)
	{
...
void FDS_Write (uint32 adr, uint8 val){
	...
	case 0x4080:
	  ...
	  else
	  {
		if (FDS_SOUND.envelope_speed)
		  ...
		  FDS_SOUND.ecounter_incr[0] = 0.0;
	  }
	  break;

	case 0x4082:
	  FDS_SOUND.freq[0] = (FDS_SOUND.freq[0] & 0xf00) | (val & 0xff);
	  FDS_SOUND.incr[0] = FDS_SOUND.geta * FDS_SOUND.freq[0] / 65536.0;
	  break;
	...
	case 0x4086:
	  ...
	  FDS_SOUND.incr[1] = FDS_SOUND.geta * FDS_SOUND.freq[1] / 65536.0;
	  break;
